...
// Regroupé en une seule fonction serverless (routing dynamique Vercel) pour
// rester sous la limite de 12 fonctions du plan Hobby. Les URLs publiques sont
// INCHANGÉES — /api/google/{auth,callback,status,disconnect} — car `[action]`
// capture le dernier segment : req.query.action vaut 'auth' | 'callback' | …
// Le redirect URI déclaré côté Google (/api/google/callback) répond donc au
// caractère près comme avant. Logique et sécurité identiques aux 4 routes
// d'origine (state anti-CSRF usage unique + expiration 10 min, refresh token
