...
// Endpoints SIGNATAIRE (publics) du flow « signature par lien intégré »,
// regroupés en une seule fonction serverless (limite Vercel Hobby) et routés
// par MÉTHODE HTTP :
//   GET  /api/sign?token=…      → charge la demande de signature   (ex-api/sign.js)
//   POST /api/submit-signature  → soumet la signature/refus         (ex-api/submit-signature.js)
//
// Le POST arrive ici via le rewrite vercel.json
//   { "source": "/api/submit-signature", "destination": "/api/sign" }
// Les URLs publiques restent STRICTEMENT identiques : des liens de signature déjà
// envoyés à des clients pointent sur /api/sign et /api/submit-signature.
// Regroupement purement structurel : logique et réponses inchangées par rapport
// aux deux handlers d'origine.
...
// GET — lecture d'une demande de signature via Firestore REST API, authentifiée
// par compte de service (IAM) : la collection signatureRequests n'est pas
// accessible directement depuis un client Firestore non authentifié.
...
// POST — soumission de la signature ou du refus par le signataire.
